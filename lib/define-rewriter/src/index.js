...
DefineRewriter.prototype.processString = function(string, relativePath) {
  ...
  graph.eagerSet.names.forEach(function (name) {
    ...
    for (var i = 0; i < src.length; i++) {
      var line = src[i];
      line = line.replace(/mainContext/, 'Em.__global');
      line = line.replace(/\s(require|requireModule)\(/, 'Em.__loader.require(');
      buffer.push(line);
    }
  });
